...
import torchaudio
import utils
from meldataset_prior_length import mel_spectrogram, load_wav, MAX_WAV_VALUE, parse_filelist
...
def inference(a):
    ...
    utmos_predictor = torch.hub.load("tarepan/SpeechMOS:v1.2.0", "utmos22_strong", trust_repo=True).cuda()
    ...
    wavs_test = []
    wavs_test += sorted(glob('audio_reconstruct_universal_testset_v2/speech/**/*.wav', recursive=True))  
    ...
    for source_path in tqdm.tqdm(wavs_test, desc="synthesizing each utterance"):
        ...
        mel = mel_spectrogram(audio, hps.data.filter_length, hps.data.n_mel_channels,
                                hps.data.sampling_rate, hps.data.hop_length, hps.data.win_length,
                                hps.data.mel_fmin, hps.data.mel_fmax, center=False)


        assert audio.shape[1] == mel.shape[2] * hps.data.hop_length, "audio shape {} mel shape {}".format(audio.shape, mel.shape)

        with torch.no_grad():
            embs = Encodec(audio)
            resynthesis_audio = model(audio, embs, n_timesteps=a.iter, temperature=a.noise_scale, solver=a.solver, sway=a.sway, sway_coef=a.sway_coef,  s_w=a.s_w, b_w=a.b_w)
            ...
            mel_hat = mel_spectrogram(resynthesis_audio.squeeze(1), hps.data.filter_length, hps.data.n_mel_channels,
                                hps.data.sampling_rate, hps.data.hop_length, hps.data.win_length,
                                hps.data.mel_fmin, hps.data.mel_fmax, center=False)
            ...
            output_file = os.path.join('periodwave_encodec_base_turbo_final_590k_rfwave_speech'+'_'+str(a.solver)+'_'+str(a.bw)+'_'+str(a.iter)+'_'+str(a.noise_scale)+'_'+str(a.s_w)+'_'+str(a.b_w)+'_sway_'+str(a.sway), file_name)
    ...
    utmos = utmos/i
    ssl_mos = ssl_mos/i
    with open(os.path.join('periodwave_encodec_base_turbo_final_590k_rfwave_speech'+'_'+str(a.solver)+'_'+str(a.bw)+'_'+str(a.iter)+'_'+str(a.noise_scale)+'_'+str(a.s_w)+'_'+str(a.b_w)+'_sway_'+str(a.sway), 'score_list.txt'), 'w') as f:
        f.write('periodwave_encodec_base_turbo_final_590k_rfwave_speech Solver:{}\nIter: {}\nNoise_scale: {}\n'.format(a.solver, a.iter, a.noise_scale))
        f.write('bw: {}\n'.format(a.bw))
        f.write('s_w: {}\nb_w: {}\n'.format(a.s_w, a.b_w))
        f.write('Sway: {}\n'.format(a.sway))
        f.write('UTMOS: {}\n'.format(utmos.item()))
...
def main():
    ...
    parser.add_argument('--ckpt', default='logs/periodwave_encodec_turbo_universe_mel45_from_speechonly470k/G_590000.pth')
