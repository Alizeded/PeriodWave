...
from model.periodwave_encodec_utils import MultiPeriodGenerator, MelSpectrogramUpsampler, FinalBlock

LRELU_SLOPE = 0.1
from math import sqrt
from model.ms_mel_loss import MultiScaleMelSpectrogramLoss 
...
class FlowMatch(BaseModule):
    def __init__(self, n_mel=512, periods=[1,2,3,5,7], noise_scale=0.25, final_dim=32, hidden_dim=512):
        ...
        self.msmel = MultiScaleMelSpectrogramLoss(sampling_rate=24000)

    def forward(self, y, embs, n_timesteps, temperature=1.0, solver='euler'): 
    ...
    def compute_loss(self, x1, emb, length, tuning_steps, temperature): 
        b, _, t = emb.shape
        x_mask = self.sequence_mask(length, x1.size(1)).to(x1.dtype)
    
        x1 = x1.unsqueeze(1)*x_mask
 
        pred_x1 = self.forward(x1, emb, n_timesteps=tuning_steps, temperature=temperature)
