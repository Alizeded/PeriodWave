...
def inference(a):
    ...
    for source_path in tqdm.tqdm(wavs_test, desc="synthesizing each utterance"):
        ...
        with torch.no_grad():
            ...
            resynthesis_audio = model(audio, mel, target_std.unsqueeze(0), n_timesteps=a.iter, temperature=a.noise_scale, solver=a.solver)
            ...
            output_file = os.path.join('periodwave_turbo_libritts_dev'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale), file_name)
    ...
    with open(os.path.join('periodwave_turbo_libritts_dev'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale), 'score_list.txt'), 'w') as f:
        f.write('periodwave_turbo_libritts_dev Solver:{}\nIter: {}\nNoise_scale: {}\n'.format(a.solver, a.iter, a.noise_scale))
        f.write('UTMOS: {}\n'.format(utmos))
...
def main():
    ...
    parser.add_argument('--ckpt', default='logs/periodwave_turbo_4_msmel_45_mel_gan_2e5/G_274000.pth')
