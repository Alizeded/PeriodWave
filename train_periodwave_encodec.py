...
from model.periodwave_encodec import FlowMatch
...
def run(rank, n_gpus, hps):
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(rank, epoch, hps, model, optimizer,
                                scaler,
...
def train_and_evaluate(rank, epoch, hps, nets, optims, scaler, loaders, logger, writers, n_gpus, Encodec):
    model = nets
    optimizer = optims
    ...
    for batch_idx, (y, length) in enumerate(train_loader):
        ...
        if  n_gpus > 1:
            loss_fm = model.module.compute_loss(y, embs, length)
        else:
            loss_fm = model.compute_loss(y, embs, length)
...
def evaluate(hps, model, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (mel, y, _) in enumerate(eval_loader):
          
            y = y.cuda(0)
            mel = mel.cuda(0)


            y_gen = model(y, mel, n_timesteps=16, temperature=1.0)
    ...
    scalar_dict = {"val/mel": val_err_tot, "val/pesq": val_pesq_tot, "val/mrstft": val_mrstft_tot}
    utils.summarize(
        writer=writer_eval,
        global_step=global_step,
        images=image_dict,
