...
def inference(a):
    ...
    wavs_test = []
    wavs_test += sorted(
        glob("audio_reconstruct_universal_testset_v2/vocals/**/*.wav", recursive=True)
    )
    ...
    for source_path in tqdm.tqdm(wavs_test, desc="synthesizing each utterance"):
        ...
        with torch.no_grad():
            ...
            output_file = os.path.join(
                "periodwave_encodec_base_turbo_step2_400k_rfwave_vocal"
                + "_"
                + str(a.solver)
                + "_"
                + str(a.bw)
    ...
    with open(
        ...
        f.write(
            "periodwave_encodec_base_turbo_step2_400k_rfwave_vocal Solver:{}\nIter: {}\nNoise_scale: {}\n".format(
