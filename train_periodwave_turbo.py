...
from model.periodwave_turbo import FlowMatch
...
def run(rank, n_gpus, hps):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=hps.train.batch_size, num_workers=16,  shuffle=False,
    ...
    optim_d = torch.optim.AdamW(itertools.chain(mrd.parameters(), mpd.parameters()), hps.train.learning_rate)
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(rank, epoch, hps, [model,utmos_model], [mpd,mrd], [optimizer,optim_d],
                                scaler,
                               [train_loader, eval_loader], logger, writer, n_gpus)
        else:
            train_and_evaluate(rank, epoch, hps, [model,utmos_model], [mpd,mrd], [optimizer,optim_d],
                                scaler,
                               [train_loader, None], None, None, n_gpus)
  
def train_and_evaluate(rank, epoch, hps, nets, discs, optims, scaler, loaders, logger, writers, n_gpus):
    ...
    for batch_idx, (mel, y, target_std, length) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        if  n_gpus > 1:
            loss_msmel, fake_y = model.module.compute_loss(y, mel, target_std, length, hps.train.tuning_steps, hps.train.finetuning_temperature)
      
        else:
            loss_msmel, fake_y = model.compute_loss(y, mel, target_std, length, hps.train.tuning_steps, hps.train.finetuning_temperature)
        ...
        loss_gen_all = loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_msmel*hps.train.w_stft
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                scalar_dict = {"loss/g/total": loss_gen_all, "learning_rate": lr, 
                               "grad_norm_g": grad_norm_g,  "grad_norm_mpd": grad_norm_mpd,  "grad_norm_mrd": grad_norm_mrd}
                scalar_dict.update({"loss/g/msmel": loss_msmel, "loss/g/mpd_gen": loss_gen_f,"loss/g/mrd_gen": loss_gen_s,"loss/g/mpd_fm": loss_fm_f,"loss/g/mrd_fm": loss_fm_s})
