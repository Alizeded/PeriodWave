...
def inference(a):
    ...
    for mel_path in tqdm.tqdm(mels, desc="synthesizing each utterance"):
        ...
        with torch.no_grad():
            ...
            output_file = os.path.join('periodwave_ardit'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale), file_name)
    ...
    with open(os.path.join('periodwave_ardit'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale), 'score_list.txt'), 'w') as f:
        f.write('periodwave_ardit Solver:{}\nIter: {}\nNoise_scale: {}\n'.format(a.solver, a.iter, a.noise_scale))
        f.write('UTMOS: {}\n'.format(utmos))
