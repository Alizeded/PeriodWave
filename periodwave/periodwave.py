...
class VectorFieldEstimator(BaseModule):
    def __init__(self, n_mel, periods, final_dim=32, hidden_dim=512):
        ...
        self.time_pos_emb = SinusoidalPosEmb(
            hidden_dim // 2
        )  # We recommend using hidden_dim instead of hidden_dim//2. Please refer the code of encodec version
        self.period_emb = nn.Embedding(
            self.len_periods, hidden_dim // 2
        )  # We recommend using hidden_dim instead of hidden_dim//2. Please refer the code of encodec version
        torch.nn.init.normal_(
            self.period_emb.weight, 0.0, (hidden_dim // 2) ** -0.5
        )  # We recommend using hidden_dim instead of hidden_dim//2. Please refer the code of encodec version
    ...
    def forward(self, x, mel, t):
        ...
        p = (
            self.period_emb(self.period_token) * math.sqrt(self.hidden_dim)
        )  # We accidentally used hidden_dim instead of hidden_dim//2. Although there is no issue in our model, we recommend changing #21, #22, #23 lines following encodec version.
