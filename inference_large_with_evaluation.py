...
from model.periodwave_large import FlowMatch
...
def inference(a):
    ...
    for source_path in tqdm.tqdm(wavs_test, desc="synthesizing each utterance"):
        ...
        with torch.no_grad():
            ...
            output_file = os.path.join('periodwave_turbo_large_libritts_dev'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale), file_name)
    ...
    with open(os.path.join('periodwave_turbo_large_libritts_dev'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale), 'score_list.txt'), 'w') as f:
        f.write('periodwave_turbo_large_libritts_dev Solver:{}\nIter: {}\nNoise_scale: {}\n'.format(a.solver, a.iter, a.noise_scale))
        f.write('UTMOS: {}\n'.format(utmos))
