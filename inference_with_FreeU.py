...
from model.periodwave_freeu import FlowMatch
...
def inference(a):
    ...
    # temp1 = glob('/workspace/sb/dataset/LibriTTS_24k/LibriTTS/test-clean/**/*.wav', recursive=True)
    # temp2 = glob('/workspace/sb/dataset/LibriTTS_24k/LibriTTS/test-other/**/*.wav', recursive=True)
    ...
    for source_path in tqdm.tqdm(wavs_test, desc="synthesizing each utterance"):
        ...
        target_std = torch.clamp((energy - energy_min) / (energy_max - energy_min), std_min, None)
        ...
        with torch.no_grad():
            ...
            resynthesis_audio = model(audio, mel, target_std.unsqueeze(0), n_timesteps=a.iter, temperature=a.noise_scale, solver=a.solver, s_w=a.s_w, b_w=a.b_w)
            ...
            output_file = os.path.join('periodwave'+'_'+str(a.solver)+'_'+str(a.iter)+'_'+str(a.noise_scale)+'_'+str(a.s_w)+'_'+str(a.b_w), file_name)
...
def main():
    ...
    parser.add_argument('--ckpt', default='logs/periodwave_libritts_24000hz/G_1000000.pth')
    parser.add_argument('--iter', default=16,  type=int)
    parser.add_argument('--noise_scale', default=0.667,  type=float)
    parser.add_argument('--solver', default='midpoint', help="euler midpoint heun rk4")
    parser.add_argument('--s_w', default=0.9,  type=float)
    parser.add_argument('--b_w', default=1.1,  type=float)
    a = parser.parse_args()
